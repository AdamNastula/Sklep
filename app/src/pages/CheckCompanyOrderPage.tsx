...
import { useGetGetOrderCompanyStatus } from "../api/order/order";
...
function CheckCompanyOrderPage() {
  const statusMap = {
    0: [
      "Przyjęte",
  ...
  const getOrderDataQuery = useGetGetOrderCompanyStatus(
    { id: id, email: email, number: orderNumber },
    {
      query: { enabled: false },
    },
  );
  const getProductsQuery = useGetCompanyProducts({ orderId: id });
  const showToast = () => {
...
export default CheckCompanyOrderPage;
